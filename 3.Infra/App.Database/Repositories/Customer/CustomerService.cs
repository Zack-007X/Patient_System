...
namespace App.Database
{
    ...
    {
        private IBaseRepository<CustomerEntity, Guid> _repository;
        ...
        public CustomerService(
            IBaseRepository<CustomerEntity, Guid> repository, 
            IMapper mapper, 
        ...
        private CustomerEntity GetEntity(CustomerInputModel model)
        {
            return _mapper.Map<CustomerEntity>(model);
        }
        private CustomerViewModel GetDto(CustomerEntity entity)
        {
        ...
        public async Task<CustomerEntity?> GetEntityAsync(Guid id)
        {
            var entity = await _repository.GetAsync(id);
        ...
        public async Task<CustomerWithSelectionViewModel> GetWithSelectionAsync(Guid id)
        {
            var entity = await _repository.GetAsync(id);
            var i = _mapper.Map<CustomerWithSelectionViewModel>(entity);


            return i;
        }

        public async Task<CustomerWithSelectionViewModel> GetBlankItemAsync()
        {
            var i = new CustomerWithSelectionViewModel();


            return i;
        }

        public async Task<List<CustomerViewModel>> GetListBySearchAsync(CustomerSearchModel model)
        {
            var data = await (
                from m_customer in _dataContext.Customers


				where
                1 == 1 
                && (string.IsNullOrEmpty(model.name) || m_customer.name.Contains(model.name))


                orderby m_customer.created descending
                select new CustomerViewModel()
                {
                    id = m_customer.id,
                    name = m_customer.name,
                    contactInfo = m_customer.contactInfo,
                    address = m_customer.address,
                    username = m_customer.username,
                    password = m_customer.password,


                    isActive = m_customer.isActive,
                    created = m_customer.created,
                    updated = m_customer.updated
                }
        ...
        public async Task<CustomerViewModel?> InsertAsync(CustomerInputModel model, bool is_force_save)
        {
        ...
        public async Task<CustomerViewModel?> UpdateAsync(Guid id, CustomerInputModel model, bool is_force_save)
        {
            var existingEntity = await _repository.GetAsync(id);
            if (existingEntity != null)
            {
                existingEntity.name = model.name;
                existingEntity.contactInfo = model.contactInfo;
                ...
                else
                {
                    ...
                    return _mapper.Map<CustomerViewModel>(existingEntity);
                }
            }
            else
		...
		public async Task<int> UpdateMultipleAsync(List<CustomerInputModel> model, bool is_force_save)
        {
            foreach(var i in model)
            {
                ...
                existingEntity.contactInfo = i.contactInfo;
                existingEntity.address = i.address;
        ...
        {
            var all_items = _dataContext.Customers;
        ...
        public async Task<byte[]> GetReportStreamAsync(CustomerReportRequestModel model)
        {
